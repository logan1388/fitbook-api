...
router.post(
  '/',
  async (
    req: { body: { userId: any; category: any; name: any; date: any; weight: any; unit: any; count: any } },
    ...
    try {
      ...
      if (
        prevMaxWeight.length == 0 ||
        prevMaxWeight[0].weight < weight ||
        (prevMaxWeight[0].weight == weight && prevMaxWeight[0].count < count)
      ) {
      ...
      if (
        prevMaxReps.length == 0 ||
        prevMaxReps[0].count < count ||
        (prevMaxReps[0].count == count && prevMaxReps[0].weight < weight)
      ) {
...
router.post(
  '/set',
  async (
    req: { body: { userId: any; name: any } },
    res: { json: (arg0: { maxWeight: any; maxReps: any; bestSet: any }) => void }
  ) => {
    try {
